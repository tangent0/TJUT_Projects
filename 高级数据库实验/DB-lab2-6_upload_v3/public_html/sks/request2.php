...
$curs2 = OCI_parse($connection, "SELECT sum(qty_stock) FROM part");

$curs1 = OCI_parse($connection, "SELECT part_number, qty_stock FROM part");
if(OCIError($curs1))
	{
...
*/
OCI_Define_By_Name($curs1,"PART_NUMBER",$num);
OCI_Define_By_Name($curs1,"QTY_STOCK",$stock);

OCI_Define_By_Name($curs2,"SUM(QTY_STOCK)",$total_stock);

/*	4. Execution of the SQL request with an immediate commit to free locks */
OCIExecute($curs1, OCI_COMMIT_ON_SUCCESS);
$chain .= "<b>PART ID  QTY_STOCK</b><br>\n";

/*	5. Read each row from the result of the Sql request */	
while (OCIfetch($curs1))

	$chain .= "$num  &nbsp &nbsp &nbsp &nbsp &nbsp $stock<br>\n";

/*----------------------------------------------------*/
OCIExecute($curs2, OCI_COMMIT_ON_SUCCESS);
$chain .= "<p /><b>Total number of parts</b><br>\n";

while (OCIfetch($curs2))
	$chain .= "<b>$total_stock</b><br>\n";
/*----------------------------------------------------*/

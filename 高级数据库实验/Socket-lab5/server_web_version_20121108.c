...
*		the message "not found" error */
        ...
        if(is_html || is_jpeg || is_gif)
        {
            ...
            strcpy(fname,strstr(path,"/")+1);
			strcpy(strchr(fname,'.'),"");

			if(is_html)strcat(fname,".html");
			else if(is_jpeg)strcat(fname,".jpg");
			else if(is_gif)strcat(fname,".gif");
            
            //printf("%d\n",stat(fname,&path_buff));
			
			if(stat(fname,&path_buff)==0) // stat successful
			{

			 FILE *fptr; //pointer to the file 
			 if((fptr=fopen(fname,"r"))==NULL)exit(0);
			 ...
			 if(is_html==1)
			 {
				 ...
				 //not works : send(conn, fname , (path_buff).st_size, 0); //some unsensible characters
			 ...
			 else if(is_jpeg==1 || is_gif==1)
			 {
				 ...
				 while(feof(fptr)==0) //not end of file
                 {
					 ...
					 //not works : 
					 send(conn, buff, sizeof(buff), 0); //some unsensible characters
                 }
			...
			else if(stat(fname,&path_buff)==-1) // stat error
			{
